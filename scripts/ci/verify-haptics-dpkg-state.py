...
def fixed_cleanup_candidate(exc: BaseException, message: str) -> BaseException:
    if not isinstance(exc, Exception):
        return exc
    failure = DpkgStateError(message)
...
def snapshot_live_descriptors(label: str) -> frozenset[int]:
    ...
    try:
        with _TRUSTED_SCANDIR("/proc/self/fd") as entries:
            for index, entry in enumerate(entries, start=1):
                ...
                if not entry.name.isascii() or not entry.name.isdecimal():
                    raise DpkgStateError(f"{label} descriptor snapshot is malformed")
        ...
        if not isinstance(exc, Exception):
            raise
        raise DpkgStateError(f"cannot enumerate {label} descriptors") from exc
    live: set[int] = set()
...
def close_owned_descriptor(
    ...
    failure = choose_cleanup_failure(
        failure,
        DpkgStateError(f"{label} descriptor close did not converge"),
...
def acquire_owned_descriptor(
    ...
    if owner.descriptor >= 0:
        raise DpkgStateError(f"{label} descriptor owner is already populated")
    ...
    try:
        ...
        if (
            ...
            raise DpkgStateError(f"{label} returned an invalid descriptor")
        owner.descriptor = returned
        _TRUSTED_FSTAT(returned)
        if os.get_inheritable(returned):
            raise DpkgStateError(f"{label} returned an inheritable descriptor")
        return returned
    except BaseException as exc:
        ...
        for descriptor in sorted(candidates, reverse=True):
            if descriptor == owner.descriptor:
                recovery_owner = owner
            else:
                recovery_owner = DescriptorOwner(descriptor)
            close_failure, _ = close_owned_descriptor(
                recovery_owner,
                f"applied {label}",
            )
...
def pin_directory_chain(
    ...
    owner_start = len(descriptor_owners)
    try:
        owner = DescriptorOwner()
        ...
        for component in components[1:]:
            ...
            descriptor = acquire_owned_descriptor(
                ...
                "dpkg admin directory component",
            )
            metadata = os.fstat(descriptor)
            namespace = os.stat(
                component,
        ...
        if isinstance(exc, OSError):
            primary: BaseException = DpkgStateError(
                f"cannot pin dpkg admin directory path: {exc}"
            )
            primary.__cause__ = exc
        else:
            primary = exc
        cleanup_failure = settle_descriptor_owners(
            descriptor_owners[owner_start:],
            "dpkg admin directory path",
        )
        if cleanup_failure is not None:
            primary = choose_cleanup_failure(
                primary,
                cleanup_failure,
                "dpkg admin directory path cleanup also failed",
            )
        raise primary
...
def read_regular(
    ...
    directory_owners: list[DescriptorOwner] = []
    file_owners: list[DescriptorOwner] = []
    directory_chain: tuple[DirectoryChainEntry, ...] = ()
    pinned_files: list[RegularFilePin] = []
    raw: bytes | None = None
    primary: BaseException | None = None
    try:
        directory_chain = pin_directory_chain(path.parent, directory_owners)
        raw = read_regular_at(
        ...
        primary = exc
    for owners, cleanup_label in (
        (file_owners, f"{label} regular file"),
        (directory_owners, f"{label} directory chain"),
    ):
        cleanup_failure = settle_descriptor_owners(owners, cleanup_label)
        if cleanup_failure is not None:
            primary = choose_cleanup_failure(
                primary,
                cleanup_failure,
                f"{cleanup_label} cleanup also failed",
            )
    if primary is not None:
        raise primary
    if raw is None:
        raise DpkgStateError(f"{label} capture produced no bytes")
    return raw
...
def read_regular_at(
    ...
    descriptor_owners: list[DescriptorOwner] | None = None,
) -> bytes:
    ...
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
    if descriptor_owners is None:
        raise DpkgStateError(f"{label} descriptor owner is not enrolled")
    owner = DescriptorOwner()
    descriptor_owners.append(owner)
    try:
        descriptor = acquire_owned_descriptor(
            owner,
            lambda: os.open(name, flags, dir_fd=directory_descriptor),
            label,
        )
    except OSError as exc:
...
def open_directory_at(
    ...
    try:
        descriptor = acquire_owned_descriptor(
            owner,
            lambda: os.open(name, flags, dir_fd=parent_descriptor),
            label,
        )
    except OSError as exc:
    ...
    try:
        ...
        if isinstance(exc, OSError):
            primary: BaseException = DpkgStateError(
                f"cannot verify pinned {label}: {exc}"
            )
            primary.__cause__ = exc
        else:
            primary = exc
        close_failure, _ = close_owned_descriptor(owner, label)
        if close_failure is not None:
            primary = choose_cleanup_failure(
                primary,
                close_failure,
...
def capture_dpkg_state(
    ...
    directory_owners: list[DescriptorOwner] = []
    directory_chain: tuple[DirectoryChainEntry, ...] = ()
    state: DpkgState | None = None
    primary: BaseException | None = None
    try:
        directory_chain = pin_directory_chain(admin, directory_owners)
    ...
    cleanup_failure = settle_descriptor_owners(
        directory_owners,
        "dpkg admin directory chain",
    )
    if cleanup_failure is not None:
        primary = choose_cleanup_failure(
            primary,
            cleanup_failure,
            "dpkg admin directory-chain cleanup also failed",
        )
    if primary is not None:
        raise primary
    if state is None:
        raise DpkgStateError("dpkg state capture produced no state")
    return state
...
def _capture_dpkg_state_contents(
    ...
    directories: dict[str, tuple[DescriptorOwner, os.stat_result]] = {}
    directory_owners: list[DescriptorOwner] = []
    file_owners: list[DescriptorOwner] = []
    pinned_files: list[RegularFilePin] = []
    state: DpkgState | None = None
    ...
    try:
        for name, label in directory_labels.items():
            owner = DescriptorOwner()
            directory_owners.append(owner)
            metadata = open_directory_at(
                owner,
                admin_descriptor,
                name,
                expected_uid,
            ...
            directories[name] = (owner, metadata)
        triggers_descriptor = directories["triggers"][0].descriptor
        info_descriptor = directories["info"][0].descriptor
        if bounded_directory_names(
            directories["updates"][0].descriptor,
            "dpkg updates directory",
        ):
            raise DpkgStateError("dpkg journal directory is not empty")
        if bounded_directory_names(
            directories["parts"][0].descriptor,
            "dpkg parts directory",
        ):
            raise DpkgStateError("dpkg journal directory is not empty")
        ...
        primary = exc
    for owners, cleanup_label in (
        (file_owners, "dpkg state regular files"),
        (directory_owners, "dpkg state child directories"),
    ):
        cleanup_failure = settle_descriptor_owners(owners, cleanup_label)
        if cleanup_failure is not None:
            primary = choose_cleanup_failure(
    ...
    if primary is not None:
        raise primary
    if state is None:
        raise DpkgStateError("dpkg state content capture produced no state")
    return state
