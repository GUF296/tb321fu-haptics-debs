 ...
 * the chip's boot-time boost register value untouched; AB005/AB006 proved
 * playback is safe with the default PLAYCFG1 value, while the AW86937 boost
 * bit mapping has not yet been independently validated.
 */
...
static const struct aw86937_y700_reg_init aw86937_y700_android_profile[] = {
...
static int aw86937_y700_apply_android_profile_locked(struct aw86937_y700 *haptics)
{
	int err;
	size_t i;

	for (i = 0; i < ARRAY_SIZE(aw86937_y700_android_profile); i++) {
		const struct aw86937_y700_reg_init *init =
			&aw86937_y700_android_profile[i];
...
static int aw86937_y700_ram_init_locked(struct aw86937_y700 *haptics)
{
	...
	err = aw86937_y700_apply_android_profile_locked(haptics);
	if (err)
		goto release_fw;
...
static int aw86937_y700_probe(struct i2c_client *client)
{
	...
	mutex_unlock(&haptics->io_lock);
	if (err)
		dev_warn(&client->dev, "Android static haptic profile failed: %d\n",
			 err);
